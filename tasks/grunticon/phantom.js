  ...
  if( theFile ){
    ...
    if( theFile.match( /\.svg$/i ) ){
      (function(){
        ...
        datacssrules.push( "#{$" + cssprefix + filenamenoext + "} { display: inline-block; width: " + parseFloat(width) + "px; height: " + parseFloat(height) + "px; background-image: url(" + svgdatauri + "); background-repeat: no-repeat; }" );

        // add rules to png url css file (changed from .icon-file format to #{$icon-file} format)
        pngcssrules.push( "#{$" + cssprefix + filenamenoext + "} { display: inline-block; width: " + parseFloat(width) + "px; height: " + parseFloat(height) + "px; background-image: url(" + pngout + filenamenoext + ".png" + "); background-repeat: no-repeat; }" );
        ...
        page.open(  inputdir + theFile, function( status ){
          ...
          pngdatacssrules.push( "#{$" + cssprefix + filenamenoext + "} { display: inline-block; width: " + parseFloat(width) + "px; height: " + parseFloat(height) + "px; background-image: url(" +  pngdatauri + page.renderBase64( "png" ) + "); background-repeat: no-repeat; }" );
